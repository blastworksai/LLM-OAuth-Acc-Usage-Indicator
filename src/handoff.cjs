  ...
  try {
    ...
    async function readResult() {
      ...
      try {
        ...
        } else if(result.ok!==false||Object.keys(result).length!==(Object.hasOwn(result,'reason')?4:3)||
          (Object.hasOwn(result,'reason')&&!/^[A-Z][A-Z_]{1,39}$/.test(result.reason))||!['CANCELLED','SETUP_FAILED','SETUP_FAILED_CHANGED'].includes(result.code)||
