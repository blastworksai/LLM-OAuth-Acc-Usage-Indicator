...
async function resolveExecutable(lookup) {
 let file;
 try {
  ...
  return bytesRead===4 && header.equals(Buffer.from([0x7f,0x45,0x4c,0x46]))?real:null;
...
function createProviderDetector({platform=process.platform,uid=process.getuid?.(),
 ...
 resolveExecutable:resolveNative=resolveExecutable}={}) {
